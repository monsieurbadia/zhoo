//! this module is used for the syntax analysis phase of the zhoo compiler
...
pub(crate) fn syntax_report(kind: &SyntaxKind) -> ReportMessage {
  ...
  match kind {
    SyntaxKind::InvalidToken(span) => (
      ...
      vec![format!("{}", "🤖 what language are you trying to speak to me in? i only speak zhoo")],
      vec![format!("{}", "👉 please go read the doc: <doc-link>".fg(Color::help()))]
    ),
    // todo #1
    ...
    SyntaxKind::UnrecognizedToken(span, expected) => (
      ...
      vec![format!(
        "{}",
        "🤖 in other languages this character is valid but the zhoo syntax does not recognize it."
      )],
